...
def train_custom_tokenizer(tokenizer_config: TokenizerConfig) -> bool:
    ...
    try:
        ...
        tokenizer.model.save(tokenizer_config.output_dir)
        ...
        # Convert to Transformers-compatible format
        LOGGER.info("Converting to Transformers-compatible format")
        transformers_tokenizer = AutoTokenizer.from_pretrained(tokenizer_config.output_dir)
...
def evaluate_tokenizer(tokenizer_path: str) -> bool:
    ...
    try:
        ...
        LOGGER.info(f"Tokenizer evaluation completed, vocabulary size: {tokenizer.vocab_size}")
        return True
        
    except Exception as e:
        LOGGER.error(f"Tokenizer evaluation failed: {e}")
        return False
...
def main() -> bool:
    ...
    try:
        ...
        LOGGER.info("Tokenizer pipeline completed successfully")
        return True
        
    except Exception as e:
        LOGGER.error(f"Tokenizer pipeline failed: {e}")
        raise
